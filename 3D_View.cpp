...
//
    ...
    glTranslatef(-.5,0.60,0.0);
    glRotatef(_angle,0.0,1.0,0.0);
    glutSolidSphere(.2,20,20); //(GLdouble radius,GLint slices, GLint stacks);
    glPopMatrix();
...
void update(int value) {
	...
	_move -= .02;
    if(_move < -2.0){
        _move = 2; //when object goes out of display while incrementing its position,
